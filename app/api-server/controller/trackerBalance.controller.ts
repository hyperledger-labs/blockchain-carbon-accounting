import { 
    PostgresDBService, TrackerBalancePayload, QueryBundle 
...
export async function getBalances(req: Request, res: Response) {
    try {
        ...
        if(offset != undefined && limit != undefined && limit != 0) {
            const balances = await db.getTrackerBalanceRepo().selectPaginated(offset, limit, queryBundles);
            const totalCount = await db.getTrackerBalanceRepo().count(queryBundles);
...
export async function insertNewTrackerBalance(balance: TrackerBalancePayload): Promise<void> {
    const db = await PostgresDBService.getInstance()
    return db.getTrackerBalanceRepo().insert(balance);
}
