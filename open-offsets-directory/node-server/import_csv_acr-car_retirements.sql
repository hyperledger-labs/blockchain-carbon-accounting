DROP TABLE IF EXISTS csv_acr_car_retirements;

CREATE TABLE csv_acr_car_retirements (
    project_id text,
    vintage integer,
    offset_credit_serial_numbers text,
    quantity_of_offset_credits bigint,
    status_effective integer,
    project_name text,
    project_type text,
...
COPY csv_acr_car_retirements FROM STDIN WITH (FORMAT CSV, HEADER);
