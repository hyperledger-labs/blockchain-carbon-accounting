    ...
    ERC1155HolderUpgradeable
{
    ...
    uint256 public divDecimals; // decimal expansion for division
     ...
     */
    function carbonIntensity(uint256 trackerId) public view returns (uint256) {
        CarbonTrackerDetails storage trackerData = _trackerData[trackerId];
        if (trackerData.totalProductAmounts > 0) {
            return (
                getTotalEmissions(trackerId).mul(divDecimals).div(
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < trackerIds.length; i++) {
            ...
            for (uint256 j = 0; j < productIds.length; j++) {
                ...
                totalEmissions = totalEmissions.add(
                    productAmount.mul(carbonIntensity(trackerIds[i])).div(
                        divDecimals
                    )
                );
            }
        ...
        )
    {
        uint256 productConv;
        ProductDetails memory product = _productData[productId];
        productConv = divDecimals.mul(product.unitAmount).div(product.amount);
