...
class EmissionsChaincode {
  ...
  async updateEmissionsMintedToken(stub:ChaincodeStub,args:string[]):Promise<ChaincodeResponse>{
      ...
      if (args.length <2){
        logger.error(`${ErrInvalidArgument} : updateEmissionsMintedToken requires 2 or more args, but provided ${args.length}`);
        return Shim.error(stringToBytes(`${ErrInvalidArgument} : updateEmissionsMintedToken requires 2 or more args, but provided ${args.length}`));
      }
      const tokenId = args[0];
      const uuids = args.slice(1);
      try {
          await (new EmissionsRecordContract(stub)).updateEmissionsMintedToken(tokenId,uuids);
      } catch (error) {
          logger.error(error);
          return Shim.error(stringToBytes((error as Error).message));
      }
