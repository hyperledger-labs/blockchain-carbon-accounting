...
export function getCO2EmissionFactor(factor:UtilityEmissionsFactorInterface,usage:number,usageUOM:string):CO2EmissionFactorInterface{
  ...
  if (factor.percent_of_renewables.length !== 0){
    emissionsUOM = 'g';
      ...
      emissionsValue = Number(factor.co2_equivalent_emissions)*usage*getUomFactor(co2EquivalentEmissionsUOM[0])/getUomFactor(co2EquivalentEmissionsUOM[1]);
