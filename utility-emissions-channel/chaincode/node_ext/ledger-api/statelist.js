...
class StateList {
  ...
  async addState(state, uuid) {
    // await this.stub.putState('<UUID HERE>', Buffer.from(JSON.stringify(state)));
    // let key = this.stub.createCompositeKey(this.name, state.getSplitKey());
    let data = State.serialize(state);
    await this.stub.putState(uuid, data);
  }
   ...
   */
  async getState(uuid) {
    const emissionasBytes = await this.stub.getState(uuid); // get the car from chaincode state
  ...
  async getAllStateByUtilityIdAndPartyId(queryData) {
    ...
    const iterator = await this.stub.getQueryResult(stringQuery);
