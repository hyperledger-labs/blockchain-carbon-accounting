...
function createConsortium() {
  ...
  set -x
  configtxgen -profile MultiNodeEtcdRaft -channelID system-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
COMPOSE_FILE_BASE=docker/docker-compose-carbonAccounting.yaml
