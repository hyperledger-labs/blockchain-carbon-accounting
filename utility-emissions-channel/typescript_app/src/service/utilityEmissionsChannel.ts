...
export default class UtilityEmissionsChannelService {
  ...
  async recordEmission(
    ...
    try {
      ...
      const emissionRecords =
        await this.opts.utilityEmissionsGateway.getEmissionsRecords(
          fabricCaller,
          {
            utilityId: utilityId,
            partyId: partyId,
      ...
      appLogger.debug(
        `${fnTag} overlap check of data between ${fromDate} to ${thruDate}`
      );
      ...
      return await this.opts.utilityEmissionsGateway.recordEmissions(
        fabricCaller,
        {
          utilityId: utilityId,
          partyId: partyId,
  ...
  async recordAuditedEmissionsToken(
    input: Input
  ): Promise<IRecordAuditedEmissionsTokenResponse> {
    ...
    try {
      ...
      if (metadata.utilityIds.length === 0) {
        throw new ClientError(
          `${fnTag} no emissions records found; nothing to audit`,
          409
        );
      }
      ...
      const token = await this.opts.netEmissionsContractGateway.issue(
        ethCaller,
        {
          addressToIssue,
          quantity,
      ...
      await this.opts.utilityEmissionsGateway.updateEmissionsMintedToken(
        fabricCaller,
        {
          tokenId: tokenId,
          partyId: partyId,
      ...
      appLogger.debug(
        `${fnTag} failed to record Audited Emissions Token : %o`,
        error
      );
      throw error;
    }
  ...
  async getEmissionsData(
    ...
    try {
      const record = this.opts.utilityEmissionsGateway.getEmissionData(
        fabricCaller,
        uuid
      );
      await this.emissionsRecordChecksum(record);
  ...
  async getAllEmissionsDataByDateRange(
    ...
    try {
      const records =
        await this.opts.utilityEmissionsGateway.getAllEmissionsDataByDateRange(
          fabricCaller,
          {
            fromDate: fromDate,
            thruDate: thruDate,
  ...
  private async emissionsRecordChecksum(record: any) {
    ...
    if (record.url && record.url.length > 0) {
      ...
      try {
        ...
        appLogger.debug(
          `${fnTag} failed to fetch ${filename} from S3 : %o`,
          error
        );
        return;
      }
  ...
  private __validateGetEmissionsDataInput(input: Input) {
    const schema = Joi.object({
      uuid: Joi.string()
        .required()
        .error(new Error("require uuid, but provided empty")),
    });
  ...
  private __validateRecordEmissionsInput(input: Input) {
    const schema = Joi.object({
      ...
      partyId: Joi.string()
        .required()
        .error(new Error("require partyId, but not provided")),
      fromDate: Joi.string()
        .regex(
          /^(-?(?:[1-9][0-9]*)?[0-9]{4})-(1[0-2]|0[1-9])-(3[01]|0[1-9]|[12][0-9])T(2[0-3]|[01][0-9]):([0-5][0-9]):([0-5][0-9])(\\.[0-9]+)?(Z)?$/
        )
