...
export default class EthNetEmissionsTokenGateway
  implements IEthNetEmissionsTokenGateway
{
  ...
  constructor(private readonly opts: IEthNetEmissionsTokenGatewayOptions) {
    ...
    Checks.truthy(
      tokenCreatedABI,
      `EthNetEmissionsTokenGateway tokenCreated event abi`
    );
  ...
  async issue(
    ...
    try {
      ...
      throw new ClientError(
        `${fnTag} failed to invoke issue method : ${error.message}`,
        409
      );
    }
