...
export default class LedgerIntegration {
  ...
  private readonly carbonAccountingRouterV2: CarbonAccountingRouterV2;
  ...
  constructor(private readonly app: Express) {
    ...
    const pluginRegistry = new PluginRegistry({
      plugins: [this.keychainPlugin, certKeychain],
    });
    ...
    const netEmissionTokenContract = new NetEmissionsTokenNetworkContract({
      ...
      contractName:
        this.ledgerConfig.netEmissionTokenContract.contractInfo.name,
      ...
      contractAddress:
        this.ledgerConfig.netEmissionTokenContract.contractInfo.address,
    });
    ...
    this.carbonAccountingRouterV2 = new CarbonAccountingRouterV2({
      logLevel: this.ledgerConfig.logLevel,
    ...
    const orgs =
      this.ledgerConfig.utilityEmissionsChaincode.network.organizations;
  ...
  private registerRouters() {
    ...
    // /api/v2/carbonAccounting : endpoints for interacting multiple ledgers
    // fabric for utilityEmissionChannel chaincode
    // ethereum for netEmissionsToken contract
    this.app.use(
      '/api/v2/utilityemissionchannel/requestmanager',
      this.carbonAccountingRouterV2.router
    );
