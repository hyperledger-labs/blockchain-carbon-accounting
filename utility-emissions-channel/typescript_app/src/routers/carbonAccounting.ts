...
export class CarbonAccountingRouter{
    ...
    private registerHandlers(){
        this.router.post(
            ...
            [
                ...
                body("automaticRetireDate").custom((value, { req }) => {
    ...
    private async recordAuditedEmissionToken(req:Request,res:Response){
        ...
        const automaticRetireDate = req.body.automaticRetireDate
        ...
        let thruDate = 0
        let fetchedEmissionsRecords:IEmissionRecord[] = []// stores fetched emissions records for updating tokenId on fabric after auditing
        ...
        for (let uuid of emissionsRecordsToAudit){
            ...
            try {
                emission = await this.opt.utilityEmissionChannel.getEmissionsData(userId,orgName,{uuid:uuid})
        ...
        for (let emissionRecords of fetchedEmissionsRecords){
            try {
