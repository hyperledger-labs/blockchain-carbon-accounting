...
export class CarbonAccountingRouter {
  ...
  private async recordAuditedEmissionToken(req: Request, res: Response) {
    ...
    const emissionsRecordsToAudit = req.body.emissionsRecordsToAudit
      .toString()
      .split(',');
    // TODO : use fabric cactus connector to call utility emission chaincode
    this.log.debug(
      `${fnTag} fetching emissionRecord uuids=%o`,
    ...
    for (const uuid of emissionsRecordsToAudit) {
      ...
      try {
        emission = await this.opt.utilityEmissionChannel.getEmissionsData(
          userId,
          orgName,
          { uuid }
        );
    ...
    try {
      ...
      this.log.info(
        `${fnTag} failed to mint audited emission token : ${error}`
      );
    ...
    try {
      await this.opt.utilityEmissionChannel.updateEmissionsMintedToken(
        userId,
        orgName,
        { tokenId, partyId, uuids: manifestIds }
      );
    } catch (error) {
