...
let db: DocumentStore<EmissionsFactorInterface | UtilityLookupItemInterface>;
...
const parse_worksheet = (file_name: string, opts) => {
    const bar = new SingleBar(
        {
            format: 'Parsing worksheet |' + '{bar}' + '| {percentage}% || {value}/{total} rows',
            barCompleteChar: '\u2588',
            barIncompleteChar: '\u2591',
            hideCursor: true,
        },
    ...
    for (const sheet of sheet_name_list) {
        ...
        opts.verbose &&
            console.log('-- opts.skip_rows = ', opts.skip_rows, ' header_row = ', header_row);
...
const import_utility_emissions = async (opts) => {
    ...
    if (opts.file == 'all' || (opts.file == 'eGRID2018_Data_v2.xlsx' && opts.sheet == 'NRL18')) {
        ...
        for (const row of data) {
            ...
            const d: EmissionsFactorInterface = {
                ...
                net_generation: row['NERC region annual net generation (MWh)'].toString(),
                net_generation_uom: 'MWH',
                ...
                source: 'https://www.epa.gov/sites/production/files/2020-01/egrid2018_all_files.zip',
                non_renewables:
                    row['NERC region annual total nonrenewables net generation (MWh)'].toString(),
                renewables:
                    row['NERC region annual total renewables net generation (MWh)'].toString(),
    ...
    if (opts.file == 'all' || (opts.file == 'eGRID2018_Data_v2.xlsx' && opts.sheet == 'ST18')) {
        ...
        for (const row of data) {
            ...
            const d: EmissionsFactorInterface = {
                ...
                source: 'https://www.epa.gov/sites/production/files/2020-01/egrid2018_all_files.zip',
                non_renewables:
                    row['State annual total nonrenewables net generation (MWh)'].toString(),
    ...
    if (opts.file == 'all' || (opts.file == 'eGRID2018_Data_v2.xlsx' && opts.sheet == 'US18')) {
        ...
        for (const row of data) {
            ...
            const d: EmissionsFactorInterface = {
                ...
                source: 'https://www.epa.gov/sites/production/files/2020-01/egrid2018_all_files.zip',
                non_renewables:
                    row['U.S. annual total nonrenewables net generation (MWh)'].toString(),
    ...
    if (
        ...
        for (const row of data) {
            ...
            const utilityFactorCall = db.get(document_id) as EmissionsFactorInterface[];
            if (utilityFactorCall.length) {
                ...
                utilityFactor.co2_equivalent_emissions_uom = d.co2_equivalent_emissions_uom;
